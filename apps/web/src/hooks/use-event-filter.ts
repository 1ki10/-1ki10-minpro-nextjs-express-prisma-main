...
import type { ApiEventsResponse } from '@/types/events-response';
   ...
   try {
     ...
     if (response.success) {
      ...
      setEvents({
        ...
        page,
        totalPages: Math.ceil(eventData.length / 12)
      }); // Hapus property 'limit' karena tidak ada di interface EventsResponse
      setError(null);
    }
   } catch (error: any) {
